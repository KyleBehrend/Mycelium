    ...
    <div className="myc-app">
      <aside className="myc-sidebar">
        ...
        <div className="myc-nav-section">
          ...
          {CURRENT_USER.streams.map(sid => {
            const s = streamById(sid);
            return (
              <button key={sid} className="myc-nav-stream" onClick={() => setScreen('dashboard')}>
      ...
      <main className="myc-main">
        {screen === 'dashboard' && <Dashboard onNav={(s: string) => setScreen(s as Screen)} onToast={toast} />}
