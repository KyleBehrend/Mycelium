// Mycelium — Dashboard Screen

function Dashboard({ onNav, onToast }) {
  ...
  const feedItems = useMemo(() => {
    const items = [];
    window.CAMPAIGNS.filter(c => c.streams.some(s => me.streams.includes(s))).slice(0, 2).forEach(c => {
      items.push({ kind: 'campaign', id: c.id, when: 'Live now', data: c });
    });
    window.LEARNINGS.filter(l => l.streams.some(s => me.streams.includes(s))).slice(0, 1).forEach(l => {
      items.push({ kind: 'learning', id: l.id, when: l.when, data: l });
    });
    window.LEARNING_FEED.filter(lf => me.streams.includes(lf.stream)).slice(0, 1).forEach(lf => {
      items.push({ kind: 'news', id: lf.id, when: lf.when, data: lf });
    });
    window.SOCIAL_POSTS.filter(p => p.streams.some(s => me.streams.includes(s))).slice(0, 1).forEach(p => {
      items.push({ kind: 'social', id: p.id, when: p.when, data: p });
    });
    return items;
  }, [me]);

  const featuredCampaign = window.CAMPAIGNS.find(c => c.id === 'c-eudg');
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', gap: 24, alignItems: 'flex-end' }}>
          <div>
            ...
            <h1 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 32, fontWeight: 700, letterSpacing: -0.6, lineHeight: 1.15 }}>
              {greet}, {firstName}.<br />
              <span style={{ opacity: 0.7 }}>{featuredCampaign ? `The EFSA consultation closes in 54 days.` : 'The forest is busy today.'}</span>
            </h1>
          ...
          <div style={{ width: 220, flexShrink: 0, display: 'none' }} className="myc-hide-sm">
            <MyceliumMark size={200} color="#74C69D" />
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 28 }}>
        {[
          ...
          <Card key={i} padding="md" accent={s.tint}>
            <div className="myc-stat">
              <span className="myc-stat-num">{s.num}</span>
            </div>
      ...
      <div className="myc-grid-2">
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 14 }}>
            <h2 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 18, fontWeight: 600, letterSpacing: -0.2 }}>For you, today</h2>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card padding="md">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              <h3 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 14.5, fontWeight: 600 }}>Your follow-throughs</h3>
              <span style={{ fontSize: 11.5, color: 'var(--myc-text-2)' }}>4 open</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {window.UPCOMING_TASKS.map((t, i) => (
                <label key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i === window.UPCOMING_TASKS.length-1 ? 'none' : '1px solid var(--myc-border-soft)', cursor: 'pointer' }}>
          ...
          <Card padding="md" style={{ overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
              <h3 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 14.5, fontWeight: 600 }}>The network</h3>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--myc-text-2)', marginBottom: 8 }}>
              You're connected to 11 orgs through your streams.
            </div>
...
function FeedCard({ item, onNav, onToast }) {
  const { kind, data, when } = item;

  if (kind === 'campaign') {
    const org = window.orgById(data.org);
    return (
      <div className="myc-feed-item" onClick={() => onNav('calendar')} style={{ cursor: 'pointer' }}>
        <div className="myc-feed-icon" style={{ background: window.streamById(data.streams[0]).color }}>
    ...
    return (
      <div className="myc-feed-item" onClick={() => onNav('community')} style={{ cursor: 'pointer' }}>
        ...
        <div className="myc-feed-body">
          ...
          <div className="myc-feed-actions">
            {data.streams.map(s => <StreamBadge key={s} stream={s} />)}
  ...
  if (kind === 'news') {
    return (
      <div className="myc-feed-item" onClick={() => onNav('knowledge')} style={{ cursor: 'pointer' }}>
        ...
        <div className="myc-feed-body">
          <div className="myc-feed-meta">
            <span style={{ color: 'var(--myc-primary)', fontWeight: 600 }}>AI-summarized news</span>
            <span>·</span>
            <span>{data.source} · {when}</span>
          </div>
          <div className="myc-feed-title">{data.title}</div>
  ...
  if (kind === 'social') {
    const author = window.personById(data.user);
    return (
      <div className="myc-feed-item" onClick={() => onNav('community')} style={{ cursor: 'pointer' }}>
        ...
        <div className="myc-feed-body">
          <div className="myc-feed-meta">
            <span>Engagement request from {author.name}</span>
            <span>·</span>
            <span>{when}</span>
          </div>
