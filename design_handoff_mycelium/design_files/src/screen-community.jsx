// Mycelium — Community Screen (Social Hub + Learnings Board)

function Community({ onToast }) {
  const [tab, setTab] = useState('social');
  return (
    <div className="myc-main-inner" style={{ paddingTop: 20 }}>
      <ScreenHeader
        eyebrow="Community"
        title="Support each other's work"
...
// ========== Social Hub ==========
function SocialHubPanel({ onToast }) {
    ...
    <div className="myc-grid-2">
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {window.SOCIAL_POSTS.map(p => {
            const author = window.personById(p.user);
            const org = window.orgById(p.org);
            const isEngaged = engaged[p.id];
            const platformIcon = p.platform === 'twitter' ? 'twitter' : p.platform === 'instagram' ? 'instagram' : 'linkedin';
      ...
      <div>
        <Card padding="md" style={{ marginBottom: 16 }}>
          <h3 style={{ fontFamily: 'DM Sans', fontSize: 14.5, fontWeight: 600, margin: '0 0 4px' }}>Top engagers this month</h3>
          <div style={{ fontSize: 12, color: 'var(--myc-text-2)', marginBottom: 14 }}>The reciprocity flywheel.</div>
          {[
            { person: 'u-elena', count: 47 },
            { person: 'u-mara', count: 39 },
            { person: 'u-finn', count: 31 },
            { person: 'u-priya', count: 28 },
            { person: 'u-jonas', count: 22 },
          ].map((t, i) => {
            const p = window.personById(t.person);
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i === 4 ? 'none' : '1px solid var(--myc-border-soft)' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--myc-text-2)' }}>{window.orgById(p.org).name}</div>
                </div>
                <div style={{ fontFamily: 'DM Sans', fontSize: 16, fontWeight: 600, color: 'var(--myc-primary)' }}>{t.count}</div>
              </div>
            );
          })}
        </Card>
        ...
        <Card padding="md">
          <h3 style={{ fontFamily: 'DM Sans', fontSize: 14.5, fontWeight: 600, margin: '0 0 12px' }}>Submit your post</h3>
          <div className="myc-form-field">
            <label className="myc-form-label">Link to post</label>
            <input className="myc-form-input" placeholder="https://linkedin.com/..." />
          </div>
          <div className="myc-form-field">
            <label className="myc-form-label">What's the ask?</label>
            <textarea className="myc-form-textarea" placeholder="e.g. Repost + tag one MEP from your country" />
          </div>
...
// ========== Learnings Board ==========
function LearningsBoardPanel({ onToast }) {
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {window.LEARNINGS.map(l => {
          const author = window.personById(l.author);
              ...
              <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 12, color: 'var(--myc-text-2)' }}>· {window.orgById(author.org).name} · {l.when}</span>
                    {l.pinned && (
                  ...
                  <h3 style={{ fontFamily: 'DM Sans', fontSize: 18, fontWeight: 600, margin: '8px 0 10px', letterSpacing: -0.2, lineHeight: 1.25, textWrap: 'balance' }}>{l.title}</h3>
...
function ReactBtn({ icon, label, active, onClick }) {
  return (
