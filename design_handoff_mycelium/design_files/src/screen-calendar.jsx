// Mycelium — Campaign Calendar

function CampaignCalendar({ onToast }) {
  const [view, setView] = useState('month');
  const [filter, setFilter] = useState('all');
  const [selected, setSelected] = useState(null);
  const [showSubmit, setShowSubmit] = useState(false);

  const filtered = window.CAMPAIGNS.filter(c => filter === 'all' || c.streams.includes(filter));
...
function CalendarMonth({ campaigns, onSelect }) {
  // Render June 2026
  const monthName = 'June 2026';
  const firstDay = new Date(2026, 5, 1).getDay(); // Mon=1; getDay returns Sun=0
  const startOffset = (firstDay + 6) % 7; // Monday-first
  const daysInMonth = 30;
  const today = 4; // pretend today is June 4 to highlight EFSA + Veganuary plan
  ...
  // For each cell, find campaigns active that day
  const inJune = (day) => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <h3 style={{ fontFamily: 'DM Sans', fontSize: 18, fontWeight: 600, margin: 0 }}>{monthName}</h3>
          ...
          return (
            <div key={i} className={`myc-cal-cell ${c.kind !== 'cur' ? 'is-out' : ''} ${isToday ? 'is-today' : ''}`}>
              ...
              {events.slice(0, 3).map((e, j) => {
                const s = window.streamById(e.streams[0]);
                return (
...
function CalendarList({ campaigns, onSelect }) {
  return (
    <Card padding="md" style={{ padding: 0 }}>
      {campaigns.map((c, i) => {
        const org = window.orgById(c.org);
        const s = window.streamById(c.streams[0]);
        const start = new Date(c.start);
        return (
          <div key={c.id} onClick={() => onSelect(c)} style={{
            display: 'flex', gap: 18, alignItems: 'flex-start',
            padding: '18px 22px',
            borderBottom: i === campaigns.length - 1 ? 'none' : '1px solid var(--myc-border-soft)',
            cursor: 'pointer',
            transition: 'background 0.12s',
          }}>
            <div style={{ width: 64, textAlign: 'center', flexShrink: 0 }}>
              <div style={{ fontSize: 11.5, color: 'var(--myc-text-2)', textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600 }}>
                {start.toLocaleDateString('en', { month: 'short' })}
              </div>
              <div style={{ fontFamily: 'DM Sans', fontSize: 28, fontWeight: 700, color: 'var(--myc-primary)', lineHeight: 1 }}>
                {start.getDate()}
              </div>
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontFamily: 'DM Sans', fontSize: 16, fontWeight: 600, marginBottom: 6, color: 'var(--myc-text)' }}>{c.title}</div>
...
function CampaignDetailModal({ campaign, onClose, onToast }) {
  const org = window.orgById(campaign.org);
  const [supported, setSupported] = useState(false);
  return (
    <div onClick={onClose} style={{
      position: 'fixed', inset: 0, background: 'rgba(20, 35, 25, 0.45)',
      backdropFilter: 'blur(4px)', zIndex: 200,
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      padding: 24,
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        background: 'var(--myc-surface)', borderRadius: 14,
        width: 'min(640px, 100%)', maxHeight: '88vh', overflowY: 'auto',
        boxShadow: 'var(--myc-shadow-lg)',
      }}>
        <div style={{ padding: '22px 26px', borderBottom: '1px solid var(--myc-border-soft)', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <h2 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 22, fontWeight: 700, letterSpacing: -0.3, lineHeight: 1.2 }}>{campaign.title}</h2>
            <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
              {campaign.streams.map(s => <StreamBadge key={s} stream={s} />)}
            </div>
          </div>
...
function SubmitCampaignModal({ onClose, onToast }) {
  return (
    <div onClick={onClose} style={{
      position: 'fixed', inset: 0, background: 'rgba(20, 35, 25, 0.45)',
      backdropFilter: 'blur(4px)', zIndex: 200,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ padding: '20px 26px', borderBottom: '1px solid var(--myc-border-soft)', display: 'flex', justifyContent: 'space-between' }}>
          <h2 style={{ margin: 0, fontFamily: 'DM Sans', fontSize: 18, fontWeight: 600 }}>Submit a campaign</h2>
          <button onClick={onClose} style={{ background: 'transparent', border: 0, padding: 4, cursor: 'pointer', color: 'var(--myc-text-2)' }}><Icon name="x" size={18} /></button>
        </div>
        <div style={{ padding: '20px 26px' }}>
          <div className="myc-form-field">
            <label className="myc-form-label">Campaign title</label>
            <input className="myc-form-input" placeholder="e.g. EU Methane Reduction Push" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div className="myc-form-field">
              <label className="myc-form-label">Start date</label>
              <input className="myc-form-input" type="date" defaultValue="2026-06-15" />
            </div>
            <div className="myc-form-field">
              <label className="myc-form-label">End date</label>
              <input className="myc-form-input" type="date" defaultValue="2026-07-15" />
            </div>
          </div>
          <div className="myc-form-field">
            <label className="myc-form-label">The ask — what support do you need?</label>
            <textarea className="myc-form-textarea" placeholder="e.g. Sign the open letter, share to LinkedIn between Jun 20–30, tag your local MEP…" />
          </div>
          <div className="myc-form-field">
            <label className="myc-form-label">Streams</label>
            <div className="myc-stream-grid">
              {window.STREAMS.slice(0, 4).map(s => (
                <div key={s.id} className="myc-stream-card">
                  <span className="myc-stream-card-dot" style={{ background: s.dot }} />
                  <span className="myc-stream-card-label">{s.short}</span>
                </div>
              ))}
            </div>
          </div>
