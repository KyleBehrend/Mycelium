// Mycelium — Command Palette (⌘K)

function CommandPalette({ open, onClose, onNav, onAskAI }) {
  ...
  const items = useMemo(() => {
    ...
    const peopleItems = window.PEOPLE.slice(0, 10).map(p => ({
      kind: 'person', icon: 'users', label: p.name, sub: `${window.orgById(p.org).name} · ${p.role}`, target: 'directory', person: p, hint: 'Profile',
    }));
    const docItems = window.KNOWLEDGE_DOCS.slice(0, 6).map(d => ({
      kind: 'doc', icon: 'book', label: d.title, sub: d.source, target: 'knowledge', hint: 'Open',
    }));
    const campaignItems = window.CAMPAIGNS.slice(0, 6).map(c => ({
      kind: 'campaign', icon: 'megaphone', label: c.title, sub: window.orgById(c.org).name, target: 'calendar', hint: 'View',
    }));
            ...
            <div>
              ...
              <div className={`myc-cmdk-item ${active === 0 ? 'is-active' : ''}`}
                ...
                <div style={{ flex: 1, fontSize: 13.5 }}>
                  <span style={{ color: 'var(--myc-text-2)' }}>Ask Mycelium: </span>
            ...
            <div key={groupName}>
              ...
              {gItems.map(it => (
                <div key={it._idx} className={`myc-cmdk-item ${active === it._idx ? 'is-active' : ''}`}
                  ...
                  {it.kind === 'person' ? <Avatar person={it.person} size={24} /> :
