// Mycelium — Member Directory (with network viz as hero)

function Directory({ onToast }) {
  const [view, setView] = useState('network');
  const [search, setSearch] = useState('');
  const [streamFilter, setStreamFilter] = useState('all');
  const [selected, setSelected] = useState(null);

  const filteredPeople = window.PEOPLE.filter(p =>
    (streamFilter === 'all' || p.streams.includes(streamFilter)) &&
    (search === '' || p.name.toLowerCase().includes(search.toLowerCase()) || window.orgById(p.org).name.toLowerCase().includes(search.toLowerCase()))
  );
      ...
      {view === 'network' && (
        <Card padding="lg" style={{ position: 'relative', overflow: 'hidden', height: 540, padding: 0 }}>
          ...
          <div style={{ position: 'absolute', top: 22, left: 22, zIndex: 2, maxWidth: 280 }}>
            <h3 style={{ fontFamily: 'DM Sans', fontSize: 16, fontWeight: 600, margin: '0 0 4px' }}>The mycelial map</h3>
            <p style={{ fontSize: 12, color: 'var(--myc-text-2)', margin: 0, lineHeight: 1.5 }}>
              Green lines = organization membership. Soft lines = shared interest streams.
              Hover the orgs to see their network.
            </p>
          </div>
          <div style={{ position: 'absolute', inset: 0 }}>
            <NetworkGraph width={1100} height={540} highlight={window.CURRENT_USER.org} />
          </div>
          <div style={{ position: 'absolute', bottom: 22, right: 22, zIndex: 2, display: 'flex', gap: 16, padding: '10px 14px', background: 'rgba(255,255,255,0.9)', borderRadius: 8, backdropFilter: 'blur(4px)', border: '1px solid var(--myc-border-soft)' }}>
            <div style={{ fontSize: 11.5, color: 'var(--myc-text-2)' }}>
              <strong style={{ color: 'var(--myc-text)', fontFamily: 'DM Sans', fontSize: 20, display: 'block', lineHeight: 1 }}>12</strong>
              orgs
            </div>
            <div style={{ fontSize: 11.5, color: 'var(--myc-text-2)' }}>
              <strong style={{ color: 'var(--myc-text)', fontFamily: 'DM Sans', fontSize: 20, display: 'block', lineHeight: 1 }}>142</strong>
              members
            </div>
            <div style={{ fontSize: 11.5, color: 'var(--myc-text-2)' }}>
              <strong style={{ color: 'var(--myc-text)', fontFamily: 'DM Sans', fontSize: 20, display: 'block', lineHeight: 1 }}>14</strong>
              countries
            </div>
          </div>
        </Card>
              ...
              <Card key={p.id} padding="md" onClick={() => setSelected(p)}>
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--myc-text-2)', marginTop: 2 }}>{window.orgById(p.org).name} · {p.country}</div>
                  </div>
                </div>
                {p.title !== 'Member' && (
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--myc-warm-deep)', marginBottom: 8, letterSpacing: 0.3 }}>★ {p.title}</div>
                )}
...
function PersonModal({ person, onClose, onToast }) {
  const org = window.orgById(person.org);
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20, 35, 25, 0.45)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--myc-surface)', borderRadius: 14, width: 'min(520px, 100%)', boxShadow: 'var(--myc-shadow-lg)', overflow: 'hidden' }}>
        <div style={{ background: `linear-gradient(135deg, ${person.tint}, ${person.tint}dd)`, padding: '32px 26px 18px', color: '#fff', position: 'relative' }}>
          ...
          <div style={{ fontFamily: 'DM Sans', fontSize: 22, fontWeight: 700, marginTop: 14, letterSpacing: -0.3 }}>{person.name}</div>
        ...
        <div style={{ padding: '22px 26px' }}>
          {person.title !== 'Member' && (
            <div style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--myc-warm-deep)', marginBottom: 12, letterSpacing: 0.3, textTransform: 'uppercase' }}>★ {person.title}</div>
          )}
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 18 }}>
            {person.streams.map(s => <StreamBadge key={s} stream={s} />)}
          </div>
